...
The current version of this program is designed for use on a local computer.
Rob Gilmore has the expanded version for usage on the MCSR.
"""
# This script is designed to parse genbank files, extract the desired features, and
...
import time
import os
from Bio import SeqIO # This module is used for parsing genbank files & extracting record features.
import csv  # Read a comma delimited list.

# This prints a short description of the script. 
print("#### This script is designed to parse genbank files and extract desired features. ###" + "\n")
...
for Gene in file2:  # Loop to create gene directories. This loop includes 2 other loops. Be mindful.
    ...
    b = 'C:/Users/shutchins2/Desktop/In Progress/Code/GBK2TREE/CDS' # Output directory
...
##### Parse genbank files & write CDS to 1 fasta file per gene for multiple alignments. #####
    time.sleep(.3)
    file_count = 0
	# Loop that establishes organism list, reads genbank file, and creates/opens new fasta file.
    for Organism in org_list: 
        ...
        output_handle = open(str(Gene[0]) + "_" + Organism + "_cds.fasta", "w")  # You can also create a genbank file.
        count = 0
        for feature in record.features:  # Loop that extracts specific features and writes them to previously created file.
